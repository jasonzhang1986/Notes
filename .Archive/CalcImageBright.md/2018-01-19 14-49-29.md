...
通过 ImageIO 读取 Image，然后读取每个像素的 RGB，转换为 YUV， 然后算出每个像素的亮度，最后求取平均值。
```java
public void brightness(String path, String imageName, float threshold) {
        ...
        try {
            ...
            System.out.println("image="+imageName + " av = " + avg);
...
//                System.out.println("bright color" );
                String outputpath = "D:\\android\\ideaProject\\Demo\\bright_"+threshold;
                File output = new File(outputpath);
                if (!output.exists()) {
                    output.mkdir();
                }
                copyFile(path, outputpath+"\\"+imageName);
            } else {
                // dark color
//                System.out.println("dark color" );
                String outputpath = "D:\\android\\ideaProject\\Demo\\dark_"+threshold;
                File output = new File(outputpath);
                if (!output.exists()) {
                    output.mkdir();
                }
